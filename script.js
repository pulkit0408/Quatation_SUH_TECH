const DATA = {
    "Website Design": {
        ...
        addons: [
            ...
            { id: "maintenance", name: "Monthly Maintenance (first month)", price: 2500 }
        ],
    ...
    "ERP Software": {
        ...
        variations: [
            ...
            { id: "erp_advanced", name: "Advanced ERP (Custom Modules)", price: 250000 }
        ],
        addons: [
...
function onServiceChange() {
    ...
    meta.variations.forEach(v => {
        ...
        o.textContent = `${v.name} — ₹${v.price.toLocaleString()}`;
    ...
    meta.addons.forEach(a => {
        ...
        label.innerHTML = `<input type="checkbox" data-price="${a.price}" value="${a.id}" onchange="recalculate()" /> <span>${a.name} (₹${a.price.toLocaleString()})</span>`;
...
function recalculate() {
    ...
    const basePrice = variationOption ? parseFloat(variationOption.dataset.price) : 0;
    ...
    const effectiveBase = (service === 'Ludo Game' && ludoSelectedPrice > 0) ? ludoSelectedPrice : basePrice;

    // subtotal depends on service type
    let subtotal = meta.type === 'per_unit'
        ? effectiveBase * units
        : effectiveBase * (units || 1);

    // addons
    const addonChecks = Array.from(addonsContainer.querySelectorAll('input[type="checkbox"]'));
    let addonsTotal = 0;
    let selectedAddons = [];
    addonChecks.forEach(cb => {
        if (cb.checked) {
            ...
            addonsTotal += p;
            selectedAddons.push({ id: cb.value, name: cb.nextSibling.textContent.trim(), price: p });
        }
    });

    // total with multipliers (before discount)
    ...
    if (discount.amount > 0) {
        ...
        discountAmount.textContent = `-₹${formatNumber(discount.amount)}`;
    } else {
    ...
    let currencySymbol = '₹';
    if (currency === 'USD') {
        const rate = 83; // approximate
        displayTotal = (finalTotal / rate);
        currencySymbol = '$';
    }

    // update UI
    priceEl.textContent = `${currencySymbol}${formatNumber(displayTotal)}`;
    subTotalEl.textContent = `₹${formatNumber(subtotal)}`;
    addonsTotalEl.textContent = `₹${formatNumber(addonsTotal)}`;
    multTotalEl.textContent = `×${multiplier.toFixed(2)}`;
    grandTotalEl.textContent = `₹${formatNumber(finalTotal)}`;

    // breakdown list
    breakList.innerHTML = '';
    const b1 = document.createElement('div');
    b1.className = 'summary-line';
    b1.innerHTML = `<div class="small">Package</div><div class="small">${variationOption ? variationOption.textContent : '-'}</div>`;
    ...
    if (selectedAddons.length) {
        selectedAddons.forEach(a => {
            ...
            r.innerHTML = `<div class="small">${a.name.replace(/\s+\(₹.*\)$/, '')}</div><div class="small">₹${formatNumber(a.price)}</div>`;
    ...
    const existingBeforeDiscountLines = summarySection.querySelectorAll('.before-discount-line');
    existingBeforeDiscountLines.forEach(line => line.remove());
    
    // Remove any existing separators
    const existingSeparators = summarySection.querySelectorAll('.discount-separator');
    existingSeparators.forEach(sep => sep.remove());
    
    if (discount.amount > 0 && discountLine && discountLine.style.display !== 'none') {
        ...
        beforeDiscountLine.innerHTML = `<div class="small">Total Before Discount</div><div class="small">₹${formatNumber(totalBeforeDiscount)}</div>`;
...
function saveQuote() {
    const quote = {
        ...
        variationLabel: variationEl.selectedOptions[0] ? variationEl.selectedOptions[0].textContent : '',
        units: unitsEl.value,
        complexity: complexityEl.value,
        urgency: urgencyEl.value,
        addons: Array.from(addonsContainer.querySelectorAll('input[type="checkbox"]:checked')).map(c => ({ id: c.value, price: c.dataset.price })),
...
function copyShareLink() {
    const state = {
        ...
        urg: urgencyEl.value,
        addons: Array.from(addonsContainer.querySelectorAll('input[type="checkbox"]:checked')).map(c => c.value),
...
function tryLoadFromQS() {
    ...
    try {
        ...
        if (decoded.addons && decoded.addons.length) {
            Array.from(addonsContainer.querySelectorAll('input[type="checkbox"]')).forEach(cb => {
